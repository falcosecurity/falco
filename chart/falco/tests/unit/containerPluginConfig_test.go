	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, plugin := range pluginsList {
				if name, ok := plugin.(map[string]interface{})["name"]; ok && name == containerPluginName {
			...
			if found {
				...
				require.True(t, slices.Contains(loadPlugins.([]interface{}), containerPluginName))
			} else {
				testCase.expected(t, nil)
				loadPlugins := config["load_plugins"]
				require.False(t, slices.Contains(loadPlugins.([]interface{}), containerPluginName))
			}
		})
	}
}
