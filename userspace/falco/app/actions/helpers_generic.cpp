...
void falco::app::actions::print_enabled_event_sources(falco::app::state& s) {
	...
	for(const auto& src : s.enabled_sources) {
				...
				if(first_plugin == nullptr) {
					...
					if(src != falco_common::syscall_source || s.is_nodriver()) {
						falco_logger::log(falco_logger::level::WARNING,
						                  "Enabled event source '" + src +
						                          "' can be opened with multiple loaded plugins, "
						                          "will use only '" +
						                          first_plugin->name() + "'");
					}
				}
		...
		if(!first_plugin && s.is_nodriver()) {
			falco_logger::log(falco_logger::level::WARNING,
			                  "Enabled event source '" + src +
			                          "' will be opened with no driver, no event will be produced");
		}
	}
}
...
void falco::app::actions::format_described_rules_as_text(const nlohmann::json& v,
                                                         std::ostream& os) {
