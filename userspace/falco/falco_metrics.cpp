...
falco_metrics::falco_metrics(falco::app::state& state)
{
	...
	m_is_enabled = state.config->m_metrics_enabled && webserver_config.m_metrics_enabled;
...
std::string falco_metrics::to_text() const
{
	...
	for (auto* inspector: m_inspectors)
	{
		for (size_t i = 0; i < sizeof(all_driver_engines) / sizeof(const char*); i++)
		{
			...
			{
				prometheus_text += converter.convert_metric_to_text_prometheus("engine_name", "falcosecurity", "scap", {{"engine_name", all_driver_engines[i]}});
				break;
			}
		}
		...
		prometheus_text += converter.convert_metric_to_text_prometheus("falco_version", "falcosecurity", "falco", {{"falco_version", FALCO_VERSION}});
		prometheus_text += converter.convert_metric_to_text_prometheus("kernel_release", "falcosecurity", "falco", {{"kernel_release", agent_info->uname_r}});
		prometheus_text += converter.convert_metric_to_text_prometheus("evt_hostname", "falcosecurity", "falco", {{"evt_hostname", machine_info->hostname}});
		...
		{
			converter.convert_metric_to_unit_convention(metrics);
			prometheus_text += converter.convert_metric_to_text_prometheus(metrics, "falcosecurity", "falco");
		}
	}
	...
	for (auto metrics_collector: m_metrics_collectors)
	{
		...
		{
		   converter.convert_metric_to_unit_convention(metric);
		   prometheus_text += converter.convert_metric_to_text_prometheus(metric, "falcosecurity", "scap");
		}
	}
