...
bool syscall_evt_drop_mgr::process_event(std::shared_ptr<sinsp> inspector, sinsp_evt *evt) {
	...
	if(m_next_check_ts < evt->get_ts()) {
		...
		delta.n_drops_buffer_close_exit =
		        stats.n_drops_buffer_close_exit - m_last_stats.n_drops_buffer_close_exit;
		delta.n_drops_buffer_proc_exit =
		        stats.n_drops_buffer_proc_exit - m_last_stats.n_drops_buffer_proc_exit;
...
bool syscall_evt_drop_mgr::perform_actions(uint64_t now,
                                           const scap_stats &delta,
                                           bool bpf_enabled) {
	...
	for(auto &act : m_actions) {
		...
		case syscall_evt_drop_action::ALERT: {
			...
			output_fields["n_evts"] =
			        std::to_string(delta.n_evts); /* Total number of kernel side events actively
			                                         traced (not including events discarded due to
			                                         simple consumer mode in eBPF case). */
			output_fields["n_drops"] = std::to_string(
			        delta.n_drops); /* Number of all kernel side event drops out of n_evts. */
			output_fields["n_drops_buffer_total"] = std::to_string(
			        delta.n_drops_buffer); /* Total number of kernel side drops due to full buffer,
			                                  includes all categories below, likely higher than sum
			                                  of syscall categories. */
			/* Kernel side drops due to full buffer for categories of system calls. Not all system
			 * calls of interest are mapped into one of the categories. Insights: (1) Identify
			 * statistical properties of workloads (e.g. ratios between categories). (2) Data-driven
			 * optimization opportunity for kernel side filtering and prioritization. (3) Response:
			 * Coarse grained insights into syscalls dropped. (4) Bonus: Cost associated with
			 ...
			 * interest, not all other system calls that did not match a category from above. Ideal
			...
			output_fields["n_drops_buffer_close_exit"] =
			        std::to_string(delta.n_drops_buffer_close_exit);
			output_fields["n_drops_buffer_proc_exit"] =
			        std::to_string(delta.n_drops_buffer_proc_exit);
			output_fields["n_drops_scratch_map"] = std::to_string(
			        delta.n_drops_scratch_map); /* Number of kernel side scratch map drops. */
			output_fields["n_drops_page_faults"] =
			        std::to_string(delta.n_drops_pf); /* Number of kernel side page faults drops
			                                             (invalid memory access). */
			output_fields["n_drops_bug"] = std::to_string(
			        delta.n_drops_bug); /* Number of kernel side bug drops (invalid condition in the
