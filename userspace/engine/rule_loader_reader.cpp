...
#define THROW(cond, err, ctx)                                                                  \
	{                                                                                          \
		if((cond)) {                                                                           \
			throw rule_loader::rule_load_exception(falco::load_result::LOAD_ERR_YAML_VALIDATE, \
			                                       (err),                                      \
			                                       (ctx));                                     \
		}                                                                                      \
	}
...
template<typename T>
static void decode_val_generic(const YAML::Node& item,
                               const char* key,
                               T& out,
...
template<typename T>
void rule_loader::reader::decode_val(const YAML::Node& item,
                                     const char* key,
                                     T& out,
                                     const rule_loader::context& ctx) {
	bool optional = false;
...
template<typename T>
void rule_loader::reader::decode_optional_val(const YAML::Node& item,
                                              const char* key,
                                              T& out,
                                              const rule_loader::context& ctx) {
	bool optional = true;
...
template void rule_loader::reader::decode_optional_val<bool>(const YAML::Node& item,
                                                             const char* key,
                                                             bool& out,
                                                             const rule_loader::context& ctx);
...
template<typename T>
static void decode_seq(const YAML::Node& item,
                       const char* key,
...
template<typename T>
static void decode_items(const YAML::Node& item,
                         std::vector<T>& out,
                         const rule_loader::context& ctx) {
	bool optional = false;

	std::function<void(T)> inserter = [&out](T value) { out.push_back(value); };

	decode_seq(item, "items", inserter, ctx, optional);
}

template<typename T>
static void decode_tags(const YAML::Node& item, std::set<T>& out, const rule_loader::context& ctx) {
	bool optional = true;

	std::function<void(T)> inserter = [&out](T value) { out.insert(value); };

	decode_seq(item, "tags", inserter, ctx, optional);
}

template<typename T>
static void decode_tags(const YAML::Node& item,
                        std::optional<std::set<T>>& out,
                        const rule_loader::context& ctx) {
	std::set<T> decoded;
...
static void decode_overrides(const YAML::Node& item,
	...
	for(YAML::const_iterator it = val.begin(); it != val.end(); ++it) {
		...
		bool is_overridable_append =
		        overridable_append.find(it->first.as<std::string>()) != overridable_append.end();
		bool is_overridable_replace =
		        overridable_replace.find(it->first.as<std::string>()) != overridable_replace.end();

		if(operation == "append") {
			rule_loader::context keyctx(it->first,
			                            rule_loader::context::OVERRIDE,
			                            key,
			                            overridectx);
			THROW(!is_overridable_append,
			      std::string("Key '") + key +
			              std::string("' cannot be appended to, use 'replace' instead"),
			      keyctx);
			...
			rule_loader::context keyctx(it->first,
			                            ...
			                            key,
			                            overridectx);
			THROW(!is_overridable_replace,
			      std::string("Key '") + key + std::string("' cannot be replaced"),
			      keyctx);

			out_replace.insert(key);
		} else {
			rule_loader::context operationctx(it->second,
			                                  rule_loader::context::VALUE_FOR,
			                                  key,
			                                  overridectx);
...
static void decode_exception_info_entry(const YAML::Node& item,
                                        const char* key,
	...
	rule_loader::context valctx(val,
	                            rule_loader::context::VALUE_FOR,
	                            (key == NULL ? "" : key),
	                            ctx);
	...
	if(val.IsScalar()) {
		THROW(val.Scalar().empty(), "Value must be non-empty", valctx);
		out.is_list = false;
		THROW(!YAML::convert<std::string>::decode(val, out.item),
		      "Could not decode scalar value",
		      valctx);
	}
	if(val.IsSequence()) {
		out.is_list = true;
...
static void decode_exception_fields(const YAML::Node& item,
                                    rule_loader::rule_exception_info::entry& out,
                                    const rule_loader::context& ctx,
                                    bool optional) {
...
static void decode_exception_comps(const YAML::Node& item,
                                   rule_loader::rule_exception_info::entry& out,
                                   const rule_loader::context& ctx) {
	bool optional = true;
...
static void decode_exception_values(const YAML::Node& item,
                                    rule_loader::rule_exception_info::entry& out,
                                    const rule_loader::context& ctx) {
	bool optional = false;
...
static void read_rule_exceptions(rule_loader::configuration& cfg,
                                 const YAML::Node& item,
	...
	for(auto& ex : exs) {
		...
		if(exvals.IsDefined()) {
			rule_loader::context vals_ctx(exvals,
			                              rule_loader::context::EXCEPTION_VALUES,
			                              "",
			                              ex_ctx);
			THROW(!exvals.IsSequence(), "Rule exception values must be a sequence", vals_ctx);
...
inline static bool check_update_expected(std::set<std::string>& expected_keys,
	...
	THROW(expected_keys.find(key) == expected_keys.end(),
	      std::string("An ") + override_type + " override for '" + key + "' was specified but '" +
	              key + "' is not defined",
	      ctx);
...
void rule_loader::reader::read_item(rule_loader::configuration& cfg,
	...
	if(item["required_engine_version"].IsDefined()) {
		...
		try {
			...
			THROW(!v.version.is_valid(),
			      "Unable to parse engine version '" + ver +
			              "' as a semver string. Expected \"x.y.z\" semver format.",
			      ctx);
		}
		...
		for(const YAML::Node& plugin : req_plugin_vers) {
			...
			rule_loader::context tmp(plugin,
			                         rule_loader::context::REQUIRED_PLUGIN_VERSIONS_ENTRY,
			                         "",
			                         ctx);
			THROW(!plugin.IsMap(), "Plugin version must be a mapping", tmp);
			decode_val(plugin, "name", r.name, tmp);
			rule_loader::context pctx(plugin,
			                          rule_loader::context::REQUIRED_PLUGIN_VERSIONS_ENTRY,
			                          r.name,
			                          ctx);
			...
			if(alternatives.IsDefined()) {
				...
				for(const auto& req : alternatives) {
					tmp = rule_loader::context(
					        req,
					        rule_loader::context::REQUIRED_PLUGIN_VERSIONS_ALTERNATIVE,
					        "",
					        pctx);
		...
		// Since a list only has items, if we have chosen to append them we can append the entire
		// object otherwise we just want to redefine the list.
		...
		v.cond_ctx = rule_loader::context(item["condition"],
		                                  rule_loader::context::MACRO_CONDITION,
		                                  "",
		                                  ctx);
		...
		// Since a macro only has a condition, if we have chosen to append to it we can append the
		// entire object otherwise we just want to redefine the macro.
		...
		std::set<std::string> override_append, override_replace;
		std::set<std::string> overridable_append{"condition",
		                                         "output",
		                                         "desc",
		                                         "tags",
		                                         "exceptions"};
		std::set<std::string> overridable_replace{"condition",
		                                          "output",
		                                          "desc",
		                                          "priority",
		                                          "capture",
		...
		if(has_overrides) {
			...
			if(has_overrides_replace) {
				...
				if(check_update_expected(expected_keys,
					...
					rule_loader::context prictx(item["priority"],
					                            rule_loader::context::RULE_PRIORITY,
					                            "",
					                            ctx);
					falco_common::priority_type parsed_priority;
					THROW(!falco_common::parse_priority(priority, parsed_priority),
					      "Invalid priority",
					      prictx);
			...
			for(const auto& key : expected_keys) {
				...
				THROW(true,
				      "Unexpected key '" + key +
				              "': no corresponding entry under 'override' is defined.",
				      keyctx);
			}
			...
			if(item["condition"].IsDefined()) {
				v.cond_ctx = rule_loader::context(item["condition"],
				                                  rule_loader::context::RULE_CONDITION,
				                                  "",
				                                  ctx);
				decode_val(item, "condition", v.cond, ctx);
			}
			...
			if(!item["condition"].IsDefined() && !item["output"].IsDefined() &&
			   !item["desc"].IsDefined() && !item["priority"].IsDefined()) {
				...
				decode_val(item, "output", v.output, ctx);
				v.output_ctx = rule_loader::context(item["output"],
				                                    rule_loader::context::RULE_OUTPUT,
				                                    "",
				                                    ctx);
...
bool rule_loader::reader::read(rule_loader::configuration& cfg,
                               collector& collector,
	...
	try {
		...
		cfg.res->add_error(falco::load_result::LOAD_ERR_YAML_PARSE,
	...
	for(auto doc = docs.begin(); doc != docs.end(); doc++) {
		if(doc->IsDefined() && !doc->IsNull()) {
			try {
				THROW(!doc->IsMap() && !doc->IsSequence(), "Rules content is not yaml", ctx);

				THROW(!doc->IsSequence(), "Rules content is not yaml array of objects", ctx);
				...
				cfg.res->add_error(falco::load_result::LOAD_ERR_VALIDATE,
